...
static void InitHardUart(void)
{
	...
	SET_BIT(USART2->CR1, USART_CR1_RXNEIE);	/* 使能RX接受中断 */
#endif
	...
	SET_BIT(USART3->CR1, USART_CR1_RXNEIE);	/* 使能RX接受中断 */
#endif
...
static void UartIRQ(UART_T *_pUart)
{
	...
	/* 清除中断标志 */
	CLEAR_BIT(_pUart->uart->SR, USART_SR_TC);
	CLEAR_BIT(_pUart->uart->SR, USART_SR_RXNE);
